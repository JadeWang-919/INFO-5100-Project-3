...
(async function loadAndVisualizeData() {
  ...
  const alcoholData = await d3.csv(
  ...
  function updateMap(selectedYear, selectedType) {
    ...
    const unemploymentByState = Object.fromEntries(
      yearData.map((d) => [
        d.state.toLowerCase(),
        d["Unemployed - Percent of Labor Force"],
      ])
    );
    ...
    gStates
      ...
      .each(function (d) {
        ...
        const alcohol = alcoholData.find(
          (record) =>
            record.state.toLowerCase() === stateName &&
      ...
      .on("click", (event, d) => {
        ...
        if (!isSelected) {
          ...
          const currentState = d3.select(event.currentTarget);
          currentState
            .raise() // Bring the selected state to the front
          ...
          const alcoholInfo = alcoholData.find(
            (record) =>
              record.state.toLowerCase() === stateNameLower &&
              record.year === selectedYear
          );
          const selectedConsumptionLabel = consumptionLabels[selectedType];
          const selectedValue = alcoholInfo
            ? alcoholInfo[selectedType]
            : "No data";
          const formattedValue =
            selectedValue !== "No data"
              ? selectedValue.toFixed(3) + " gallons per person"
              : "No data";
  ...
  function updateLegend(minVal, maxVal) {
    ...
    gradient.select(".end").attr("stop-color", unemploymentColorScale(minVal));
