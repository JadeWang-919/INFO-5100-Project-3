document.addEventListener("DOMContentLoaded", async function() {
    ...
    chartArea.append("text")
        ...
        .attr("x", -chartHeight / 2)
        .attr("y", chartWidth + margin.right - 20)
        .attr("text-anchor", "middle")
        .attr("transform", "rotate(-90)")
        .style("font-weight", "bold")
    ...
    function renderGridlines() {
        ...
        const yGrid = d3.axisLeft(yScaleUnemployment)
            .tickSize(-chartWidth)
            .tickFormat('');
    ...
    async function loadData() {
        try {
            ...
            d3.select("#loading-message").text("Error loading data. Please check console for details.");
    ...
    function handleZoom(event) {
        ...
        const xGrid = d3.axisBottom(new_xScale)
            .tickSize(-chartHeight)
            .tickFormat('');
        chartArea.select(".x-grid").call(xGrid);

        const yGrid = d3.axisLeft(new_yScaleUnemployment)
            .tickSize(-chartWidth)
            .tickFormat('');
        chartArea.select(".y-grid").call(yGrid);

        chartArea.select("#viewport").selectAll(".line").attr("transform", event.transform);
        chartArea.select("#viewport").selectAll(".point").attr("transform", event.transform);
